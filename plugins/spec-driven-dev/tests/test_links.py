...
from conftest import run_spectl, SPECTL
...
def test_link_basic(tmp_path):
    ...
    rc, out, err = run_spectl("link", str(change_a), str(change_b), cwd=tmp_path)
    assert rc == 0
...
def test_link_idempotent(tmp_path):
    ...
    run_spectl("link", str(change_a), str(change_b), cwd=tmp_path)
    rc, out, err = run_spectl("link", str(change_a), str(change_b), cwd=tmp_path)
    assert rc == 0
...
def test_link_nonexistent_path(tmp_path):
    ...
    rc, out, err = run_spectl("link", str(change_a), str(tmp_path / "nope"), cwd=tmp_path)
    assert rc == 1
    assert "not found" in err.lower()


# spec: spectl requirement=link-changes scenario=spec-root-derivation
def test_spec_root_derivation(tmp_path):
    """Verify derive_spec_root returns the correct path."""
    import importlib.util
    spec = importlib.util.spec_from_file_location("spectl", SPECTL)
    spectl = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(spectl)

    change_path = Path("/foo/bar/specs/changes/my-change")
    result = spectl.derive_spec_root(change_path)
    assert result == Path("/foo/bar/specs")
...
def test_unlink_basic(tmp_path):
    change_a, change_b, _, _ = make_monorepo(tmp_path)
    run_spectl("link", str(change_a), str(change_b), cwd=tmp_path)
    rc, out, err = run_spectl("unlink", str(change_a), str(change_b), cwd=tmp_path)
    assert rc == 0
...
def test_unlink_removes_links_key(tmp_path):
    change_a, change_b, _, _ = make_monorepo(tmp_path)
    run_spectl("link", str(change_a), str(change_b), cwd=tmp_path)
    run_spectl("unlink", str(change_a), str(change_b), cwd=tmp_path)
...
def test_validate_broken_link_missing_specs(tmp_path):
    ...
    rc, out, err = run_spectl("validate", "--dir", str(specs_a), cwd=tmp_path)
    assert rc == 1
...
def test_validate_fix_asymmetric_link(tmp_path):
    ...
    rc, out, err = run_spectl("validate", "--fix", "--dir", str(specs_a), cwd=tmp_path)
    assert rc == 0
...
def test_info_with_resolved_links(tmp_path):
    ...
    rc, out, err = run_spectl("info", "add-oauth", "--dir", str(specs_a), cwd=tmp_path)
    assert rc == 0
    assert "links:" in out
    assert "def34" in out
...
def test_info_with_links_json(tmp_path):
    ...
    rc, out, err = run_spectl("info", "add-oauth", "--json", "--dir", str(specs_a), cwd=tmp_path)
    assert rc == 0
    data = json.loads(out)
...
def test_archive_warns_active_links(tmp_path):
    ...
    rc, out, err = run_spectl(
        "archive", "add-oauth", "--force", "--dir", str(specs_a), cwd=tmp_path
    )
    assert rc == 0
...
def test_archive_rejected_still_warns(tmp_path):
    ...
    rc, out, err = run_spectl(
        "archive", "add-oauth", "--rejected", "--dir", str(specs_a), cwd=tmp_path
    )
    assert rc == 0
    assert "Linked changes still active:" in out
...
def test_full_lifecycle(tmp_path):
    ...
    rc, out, err = run_spectl("info", "add-token-refresh", "--dir", str(specs_b), cwd=tmp_path)
    assert rc == 0
    assert "abc12" in out
    ...
    rc, out, err = run_spectl("validate", "--dir", str(specs_b), cwd=tmp_path)
    assert rc == 0
