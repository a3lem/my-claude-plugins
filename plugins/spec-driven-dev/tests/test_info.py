...
import json
from conftest import run_spectl, make_change, make_archived_change
...
def test_info_output(spec_root):
    ...
    rc, out, err = run_spectl("info", "add-oauth", "--dir", str(spec_root), cwd=spec_root.parent)
    assert rc == 0
...
def test_info_json(spec_root):
    ...
    rc, out, err = run_spectl("info", "add-oauth", "--json", "--dir", str(spec_root), cwd=spec_root.parent)
    assert rc == 0
    data = json.loads(out)
...
def test_info_resolve_by_id(spec_root):
    ...
    rc, out, err = run_spectl("info", "x7k2m", "--dir", str(spec_root), cwd=spec_root.parent)
    assert rc == 0
...
def test_info_resolve_by_path(spec_root):
    ...
    rc, out, err = run_spectl("info", str(cp), "--dir", str(spec_root), cwd=spec_root.parent)
    assert rc == 0
    assert "add-oauth" in out
...
def test_info_archived_by_slug(spec_root):
    ...
    rc, out, err = run_spectl(
        "info", "add-oauth", "--archived", "--dir", str(spec_root),
...
def test_info_archived_by_id(spec_root):
    ...
    rc, out, err = run_spectl(
        "info", "x7k2m", "--archived", "--dir", str(spec_root),
        cwd=spec_root.parent,
    )
    assert rc == 0
    assert "archived: merged" in out
...
def test_info_no_match(spec_root):
    rc, out, err = run_spectl("info", "nonexistent", "--dir", str(spec_root), cwd=spec_root.parent)
    assert rc == 1
